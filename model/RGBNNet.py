    ...
    def __init__(self, in_channels, freeze_stage_level=0):
        ...
        with torch.no_grad():
            if in_channels >= 3:
                ...
                new_proj.weight[:, 3] = old_proj.weight[:, 0]
            else:
                # If only 1 channel input, init from red channel weights
                new_proj.weight[:, 0] = old_proj.weight[:, 0]
...
class RGBNStreamSwin(nn.Module):
    def __init__(self, num_classes, freeze_rgbn=2):
        super().__init__()
        self.rgbn_backbone = SwinBackboneWrapper(in_channels=4, freeze_stage_level=freeze_rgbn)
    ...
    def forward(self, rgb,nir):
        rgbn = torch.cat((rgb, nir), dim=1)               # [B, 4, H, W]

        feat_rgbn = self.rgbn_backbone(rgbn)  # (B, H, W, C)
        ...
        # print(feat_snir.shape) 

        return self.classifier(x)


if __name__ == "__main__":
    model = RGBNStreamSwin(num_classes=6, freeze_rgbn=2)

    rgbn = torch.randn(8, 3, 224, 224)   # RGB+NIR input
    snir = torch.randn(8, 1, 224, 224)   # slope-enhanced NIR input
    out = model(rgbn, snir)
